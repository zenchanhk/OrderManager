...
namespace AmiBroker.Controllers
{
    ...
    public class OrderManager : IndicatorBase
    {
                ...
                if (script != null)
                {
                    ...
                    foreach (Strategy strategy in script.Strategies)
                    {
                        ...
                        if (strategy.ActionType == ActionType.Long || strategy.ActionType == ActionType.LongAndShort)
                        {
                            ...
                            if (signal)
                                Task.Run(() => ProcessSignalAsync(script, strategy, OrderAction.Buy, logTime));

                            signal = ATFloat.IsTrue(strategy.SellSignal.GetArray()[BarCount - 1]);
                            if (signal)
                                Task.Run(() => ProcessSignalAsync(script, strategy, OrderAction.Sell, logTime));
                        }
                        if (strategy.ActionType == ActionType.Short || strategy.ActionType == ActionType.LongAndShort)
                        {
                            ...
                            if (signal)
                                Task.Run(() => ProcessSignalAsync(script, strategy, OrderAction.Short, logTime));

                            signal = ATFloat.IsTrue(strategy.CoverSignal.GetArray()[BarCount - 1]);
                            if (signal)
                                Task.Run(() => ProcessSignalAsync(script, strategy, OrderAction.Cover, logTime));
                        }
                    }
                }
            }
        ...
        private async Task ProcessSignalAsync(Script script, Strategy strategy, OrderAction orderAction, DateTime logTime)
        {
            Log log = new Log
            {
                Time = DateTime.Now,
                ...
                if (ValidateSignal(strategy, strategy.AccountStat[acc.Name], orderAction, out message))
                {
                    ...
                    foreach (var account in strategy.AccountsDic[orderAction])
                    {
                        ...
                        if (orderType != null)
                        {
                            ...
                            OrderLog orderLog = await account.Controller.PlaceOrderAsync(account, strategy, orderType, orderAction, BarCount - 1);
